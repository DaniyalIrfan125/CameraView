...
internal constructor(context: Context) {
  ...
  fun setOnClick(myListener: MyListener){
  ...
  /** Classifies a frame from the preview stream.  */
   fun classifyFrame(bitmap: Bitmap): Task<String> {
    if (labeler == null) {
  ...
  internal fun close() {
   ...
   fun extractDataFromFrame(frame: Frame ,Data: Any) : Task<String> {
     ...
     return labeler.processImage(getVisionImageFromFrame(frame, Data)).continueWith { task ->
       val endTime = SystemClock.uptimeMillis()
       Log.d(TAG, "Time to run model inference: " + java.lang.Long.toString(endTime - startTime))
       ...
       // print the results
       myListener!!.callback(textToShow)
       Log.d("value",textToShow);
       textToShow
     }
  }
   ...
   fun getVisionImageFromFrame(frame : Frame ,Data: Any) : FirebaseVisionImage{
    ...
    val image = FirebaseVisionImage.fromByteArray(Data as ByteArray, imageMetaData)
